...
function templateRowToProfile(row: TemplateRow): AssignmentProfile {
  return {
    id: row.id,
    name: row.name,
    serviceTier: normalizeServiceTier(row.service_tier),
    courseLevel: row.course_level,
    assignmentPrompt: row.assignment_prompt,
    assignmentRequirements: row.assignment_requirements,
    rubric: row.rubric ?? "",
    citationStyle:
      row.citation_style === "APA" ||
      row.citation_style === "MLA" ||
      row.citation_style === "None"
        ? row.citation_style
        : "APA",
    assignmentType: normalizeAssignmentType(row.assignment_type),
    feedbackFocus: normalizeFeedbackFocus(row.feedback_focus),
...
export default function DashboardClient() {
  ...
  useEffect(() => {
    async function loadSavedTemplates() {
      ...
      const { data, error } = await supabase
        .from("templates")
        .select(
          "id,name,course_level,assignment_prompt,assignment_requirements,rubric,citation_style,assignment_type,feedback_focus,instructor_name,service_tier",
        )
        .order("course_level", { ascending: true })
        .order("name", { ascending: true });

      if (error) {
        setOutput("Unable to load saved templates.");
        setIsLoadingTemplates(false);
        return;
      }

      setProfiles(((data ?? []) as TemplateRow[]).map(templateRowToProfile));
      setIsLoadingTemplates(false);
    }
  ...
  function sortProfiles(nextProfiles: AssignmentProfile[]) {
    return [...nextProfiles].sort((a, b) => {
  ...
  function buildTemplatePayload(name: string, course: string) {
    return {
      name,
  ...
  async function saveCurrentProfile() {
    ...
    const profileId = editingProfileId ?? existingProfileWithSameName?.id;
    const payload = buildTemplatePayload(trimmedName, trimmedCourseLevel);
    const supabase = createClient();
    ...
    if (error || !data) {
      setOutput("Unable to save assignment template.");
      return;
    }

    const nextProfile = templateRowToProfile(data as TemplateRow);

    await supabase.from("assignment_prompts").upsert(
      {
        template_id: nextProfile.id,
        name: trimmedName,
        course_level: trimmedCourseLevel,
        service_tier: serviceTier,
        assignment_type: assignmentType,
        assignment_prompt: assignmentPrompt,
        assignment_requirements: assignmentRequirements,
      ...
      { onConflict: "user_id,course_level,name" },
    );
    ...
    setProfiles(sortProfiles([...remainingProfiles, nextProfile]));
  ...
  async function deleteProfile(profileId: string) {
    ...
    if (error) {
      setOutput("Unable to delete assignment template.");
      return;
    }
